$(function() {
  ...
  function displayCharts(zoom) {
    ...
    coux(["view", "url_events", {
      ...
      .data([allEvents(context, "events")].concat(top10.map(function(url) {return urlEvents(context, url)})))
      ...
      $(".horizon .title").before('<br/><span>URL: </span>');
      $(".horizon .value").before('<br/><span>count: </span>');
  ...
  function urlEvents(context, url) {
  ...
  function allEvents(context, name) {
    return context.metric(function(start, stop, step, callback) {
      start = (+start)/1000, stop = (+stop)/1000, step = (+step)/1000;
      coux(["view", "events", {
